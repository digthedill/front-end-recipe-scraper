<template>
  <div class="recipe-container">
      ...
      <a :href="fullRecipe.srcUrl" target="_blank" class="og-recipe-link">Original Recipe</a>
            <br><br>

        </div>
           <div class="img-container col-sm">
      <img :src="fullRecipe.image" :alt="fullRecipe.title" class="img-fluid rounded">
        </div>
      </div>
      <hr>
      ...
      <div class="row">
          <div class="col">
            <router-link to="/recipes"><button class="btn btn-secondary">To the Cookbook</button></router-link>
          </div>
          <div class="col"><button class="btn btn-warning col-sm" @click="deleteRecipe">Delete Recipe</button></div>
      </div>
  </div>
</template>
...
export default {
    ...
    methods: {
        deleteRecipe() {
           this.recipeRef.doc(this.fullRecipe.id).delete().then(function() {
               console.log("Deleted!")
